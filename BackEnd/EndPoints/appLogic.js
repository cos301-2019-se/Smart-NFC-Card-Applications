 ...
 *  2019/09/12  Tjaart      3.0         Added deletes
 *
    ...
    async deleteVisitorPackage() {
        ...
        if (!presentParams) {
            ...
            if (!invalidParams) {
                ...
                if (visitorPackage.success) {
                    // delete client
                    await this.sharedLogic.crudController.deleteClient(visitorPackage.data.clientId);

                    // delete wallet
                    await this.sharedLogic.crudController.deleteWallet(visitorPackage.data.linkWalletId);

                    // delete wifi
                    await this.sharedLogic.crudController.deleteTempWifiAccess(visitorPackage.data.tempWifiAccessId);

                    // delete tpa
                    let tpaXRooms = await this.sharedLogic.crudController.getTPAxRoomsByTpaId(visitorPackage.data.tpaId);
                    if (tpaXRooms.success) {
                        for (let i = 0; i < tpaXRooms.data.length; i++) {
                            await this.sharedLogic.crudController.deleteTPAxRoom(tpaXRooms.data[i].tpaId, tpaXRooms.data[i].roomId);
                        }
                    }

                    // delete visitorPackage
                    let ret = await this.sharedLogic.crudController.deleteVisitorPackage(this.body.visitorPackageId);

                    this.sharedLogic.endServe(true, "Deleted visitor package", {});
                }
                else {
                    this.sharedLogic.endServe(visitorPackage.success, visitorPackage.message, visitorPackage.data);
                }
